# Crandall Christian Academy — Preschool Management Platform Build Brief
**For: Claude Code**
**Last updated: 2026-04-09**
**Status:** v2.0 — Expanded spec after competitive gap analysis against Brightwheel, Procare, Lillio, Kangarootime, Playground, Famly, Sandbox, and LineLeader. Adds 15 new feature areas. SuiteDash audit appendix to be added after read-only walkthrough.

> Read this in full before writing code. Read alongside `CCA_MARKETING_BRIEF.md`, `BRAND.md`, and `COPY.md` — same repo, same stack, same design system. The portal is **not** a separate project.
...
Replace the CCA SuiteDash workspace with a purpose-built preschool management platform that is:

1. **The best preschool management app a small Christian academy could run.** Not a SuiteDash clone. Not a Brightwheel clone. Phone-app simplicity, Apple-grade polish, zero feature gaps vs. modern competitors, with deep support for the unique needs of a faith-based school.
2. **Three-sided:** parents manage enrollment, payments, check-in, and receive daily reports; staff manage classrooms, attendance, curriculum, and time tracking; admin orchestrates everything including billing, compliance, door access, and camera oversight.
...
4. **One Next.js repo** with the marketing site. Portal lives under `app/(portal)/` route group on `portal.crandallchristianacademy.com`. Shared design system, shared Supabase project, single Vercel deploy.
...
| Layer | Choice | Why |
|---|---|---|
| Auth | **Supabase Auth** (email/password + magic link + optional WebAuthn for admin) with RLS | Same Supabase project as marketing leads. RLS enforces family-scoped and classroom-scoped data. |
...
| Payments | **Stripe** (Subscriptions for tuition + one-off Invoices for fees) | Industry standard. ACH for lower fees. Card as fallback. |
| Storage | **Supabase Storage** (private buckets per resource type) | Student photos, staff credentials, daily report photos, immunization records, incident reports. |
...
| Feature flags | **`@vercel/flags`** | Roll out door control, camera feeds, new billing features behind flags. |
...
create policy family_access on <table>
  for select using (
    ...
    or exists (select 1 from user_roles where user_id = auth.uid() and role in ('cca_owner','cca_admin'))
  );
```
Classroom-scoped tables gate by `classroom_staff_assignments`. Writes are gated by role helper functions.

---

## 5. Data model (high-level — full DDL goes in `supabase/migrations/`)

> All tables: `id uuid pk default gen_random_uuid()`, `created_at`, `updated_at` (trigger), `deleted_at` (soft delete), `created_by`, `updated_by`. RLS enabled. Generated TS types in `src/lib/database.types.ts`.

### 5.1 Identity & access
- `users` (extends `auth.users`) — first/last name, phone, avatar_path, default_role, locale, timezone.
- `user_roles` — `(user_id, role, scope_type [global/classroom/family], scope_id, granted_by, granted_at, revoked_at)`.
- `audit_log` — `(actor_id, impersonating_id, action, entity_type, entity_id, before jsonb, after jsonb, ip, user_agent, created_at)`. Append-only. Never deleted.
...
supabase/
  migrations/
    ...
    0004_families.sql
    0005_classrooms.sql
    0006_check_in_attendance.sql
    0007_daily_reports.sql
    0008_staff_payroll.sql
    0009_billing.sql
    0010_messaging.sql
    0011_curriculum.sql
    0012_door_control.sql
    0013_cameras.sql
    0014_enrollment.sql
    0015_compliance.sql
    ...
    0020_role_helpers.sql
    0021_food_program.sql
    0022_expenses.sql
    0023_portfolios.sql
    0024_leads_crm.sql
    0025_surveys.sql
    ...
    0029_checklists.sql
    0030_documents.sql
    0031_calendar_events.sql
    0032_emergency.sql
    0033_training.sql
    0034_dfps_compliance.sql
    0035_analytics.sql
    0036_newsfeed.sql
    0037_enhanced_billing.sql
    0038_entity_notes.sql
    0039_rls_policies_v2.sql
    0099_seed_dev.sql
public/
  pwa/
    icon-192.png
...
- White-label / multi-school (CCA is the only tenant).

---

## 22. Build sequence (suggested — v2.0 updated)

> v2.0 adds steps 22–35 for the new feature areas. The core build order is unchanged — new features are layered in at logical points or appended as their own phases.
