            ...
            {bookings.map((b) => (
              <div key={b.id} className="flex items-center justify-between px-4 py-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button variant="secondary" size="sm" onClick={() => alert('Cancel booking')}>
                    Cancel
                  </Button>
                </div>
              </div>
      ...
      {tab === 'slots' && (
        <div className="space-y-4">
          ...
          {showAddSlot && (
            <AddSlotForm
              onAdd={(slot) => {
                setSlots((prev) => [...prev, { ...slot, id: Date.now().toString() }])
      ...
      {tab === 'rates' && (
        <div className="space-y-4">
              ...
              {rates.map((r) => (
                <div key={r.id} className="flex items-center justify-between px-4 py-3">
                  <div>
                    ...
                    <p className="text-xs" style={{ color: 'var(--color-muted-foreground)' }}>
                      ${r.hourly_rate}/hr · {r.minimum_hours}hr minimum
                    </p>
                  </div>
                  <Button
                    variant="secondary"
                    size="sm"
                    onClick={() => setRates((prev) => prev.filter((x) => x.id !== r.id))}
                  >
                    <X className="h-3.5 w-3.5" />
                  </Button>
                </div>
          ...
          {showAddRate && (
            <AddRateForm
              onAdd={(rate) => {
                setRates((prev) => [...prev, { ...rate, id: Date.now().toString() }])
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-2 justify-end">
        ...
        <Button
          size="sm"
          onClick={() => onAdd({ weekday, start_time: startTime, end_time: endTime, capacity })}
        >
          Add
        </Button>
      </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-2 justify-end">
        ...
        <Button
          size="sm"
          disabled={!ageRange.trim()}
          onClick={() =>
            onAdd({ age_range: ageRange, hourly_rate: hourlyRate, minimum_hours: minimumHours })
          }
        >
          Add
        </Button>
