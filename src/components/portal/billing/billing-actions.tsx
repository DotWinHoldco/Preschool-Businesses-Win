...
export function BillingActions() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    alert(`${count} invoices ${mode} generated for ${scope === 'all' ? 'all families' : 'selected families'} - ${month} ${year}`)
    close()
  }
      ...
      <Dialog open={open} onOpenChange={(v) => !v && close()}>
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-[var(--color-foreground)] mb-1">Families</label>
