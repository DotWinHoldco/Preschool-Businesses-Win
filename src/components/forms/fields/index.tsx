  ...
  return (
    <div className="space-y-2">
      {options.map((o) => (
        <label
          key={o.value}
          className="flex items-center gap-3 cursor-pointer rounded-lg border p-3 transition-colors hover:bg-[var(--color-muted)]"
          style={{
            borderColor: value === o.value ? 'var(--color-primary)' : 'var(--color-border)',
            backgroundColor:
              value === o.value ? 'var(--color-primary-50, rgba(0,0,0,0.03))' : undefined,
          }}
        >
          <input
            type="radio"
...
function ImageChoiceField({ field, value, onChange, disabled }: FormFieldProps) {
  const options =
    (field.config?.options as { label: string; value: string; image_url?: string }[]) || []
      ...
      {options.map((o) => {
        ...
        return (
          <button
            ...
            className="relative rounded-lg border-2 overflow-hidden transition-all p-3 text-center"
            style={{
    ...
    <div className="flex flex-wrap gap-2">
      {options.map((o) => (
        <button
          ...
          style={{
            borderColor: value === o.value ? 'var(--color-primary)' : 'var(--color-border)',
            backgroundColor: value === o.value ? 'var(--color-primary)' : 'transparent',
            color: value === o.value ? 'var(--color-primary-foreground)' : 'inherit',
          }}
        >
          {o.label}
        </button>
    ...
    <div className="flex gap-1">
      {Array.from({ length: max }, (_, i) => i + 1).map((n) => (
        <button
          ...
          style={{ color: n <= current ? 'var(--color-warning, #F59E0B)' : 'var(--color-border)' }}
        >
          ★
        </button>
      ))}
      ...
      <div className="flex gap-1 justify-center">
        {Array.from({ length: max - min + 1 }, (_, i) => min + i).map((n) => (
          <button
            ...
            disabled={disabled}
            onClick={() => onChange(n)}
            className="w-10 h-10 rounded-md border-2 text-sm font-semibold transition-all"
            style={{
              borderColor: current === n ? 'var(--color-primary)' : 'var(--color-border)',
              backgroundColor: current === n ? 'var(--color-primary)' : 'transparent',
              color: current === n ? 'var(--color-primary-foreground)' : 'inherit',
            }}
          >
            {n}
          </button>
...
function NpsField({ field: _field, value, onChange, disabled }: FormFieldProps) {
  const current = Number(value)
  return (
    <div>
      <div className="flex gap-1 justify-center">
        {Array.from({ length: 11 }, (_, i) => i).map((n) => {
          const bg =
            current === n ? (n <= 6 ? '#EF4444' : n <= 8 ? '#F59E0B' : '#10B981') : undefined
          return (
            <button
              key={n}
              type="button"
              disabled={disabled}
              onClick={() => onChange(n)}
              className="w-9 h-9 rounded-md border-2 text-xs font-semibold transition-all"
              style={{
                borderColor: current === n ? bg : 'var(--color-border)',
                backgroundColor: bg || 'transparent',
                color: current === n ? '#fff' : 'inherit',
              }}
            >
              {n}
            </button>
...
function YesNoField({ field: _field, value, onChange, disabled }: FormFieldProps) {
  return (
    <div className="flex gap-3">
      {[
        ...
        <button
          ...
          style={{
            borderColor: value === o.val ? 'var(--color-primary)' : 'var(--color-border)',
            backgroundColor: value === o.val ? 'var(--color-primary)' : 'transparent',
            color: value === o.val ? 'var(--color-primary-foreground)' : 'inherit',
          }}
        >
          {o.label}
        </button>
  ...
  return (
    <label className="flex items-start gap-3 cursor-pointer">
      <input
        ...
        disabled={disabled}
        className="mt-1 rounded accent-[var(--color-primary)]"
      />
...
function DateField({ field: _field, value, onChange, disabled }: FormFieldProps) {
  return (
    <Input
      type="date"
      value={String(value ?? '')}
...
function TimeField({ field: _field, value, onChange, disabled }: FormFieldProps) {
  return (
    <Input
      type="time"
      value={String(value ?? '')}
...
function DateTimeField({ field: _field, value, onChange, disabled }: FormFieldProps) {
  return (
    <Input
      type="datetime-local"
      value={String(value ?? '')}
...
function FileUploadField({ field: _field, value, onChange, disabled }: FormFieldProps) {
  return (
    <div
...
function SignaturePadField({ field: _field, value: _value, onChange, disabled }: FormFieldProps) {
    ...
    <div className="space-y-3">
      <div className="flex gap-2">
        <button
          ...
          style={{
            backgroundColor: mode === 'type' ? 'var(--color-primary)' : 'transparent',
            color: mode === 'type' ? 'var(--color-primary-foreground)' : 'inherit',
          }}
        >
          Type
        </button>
        <button
          ...
          className="text-xs px-3 py-1 rounded-full border"
          style={{
            backgroundColor: mode === 'draw' ? 'var(--color-primary)' : 'transparent',
            color: mode === 'draw' ? 'var(--color-primary-foreground)' : 'inherit',
          }}
        >
          Draw
        </button>
      ...
      {mode === 'type' ? (
        <Input
          ...
          onChange={(e) => {
            ...
            onChange({ type: 'typed', name: e.target.value, timestamp: new Date().toISOString() })
          }}
          disabled={disabled}
        ...
        <div
          className="border rounded-lg h-32 flex items-center justify-center"
          style={{ borderColor: 'var(--color-border)' }}
        >
...
function SectionHeaderField({ field }: FormFieldProps) {
  return (
    <h2 className="text-xl font-bold mt-6" style={{ color: 'var(--color-foreground)' }}>
      {field.label}
    </h2>
  )
}
